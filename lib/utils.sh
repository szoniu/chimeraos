    ...
    while true; do
        ...
        case "${choice}" in
            ...
            shell)
                ...
                PS1="(chimera-installer rescue) \w \$ " bash --norc --noprofile || true
                [[ ${_stderr_redirected} -eq 1 ]] && exec 2>>"${LOG_FILE}"
                continue
                ;;
            continue)
...
# is_supported_arch — This installer is hardwired for x86_64. chimera-bootstrap,
# the apk packages, grub-x86_64-efi target and bundled x86_64 gum binary are all
# x86_64-only. On any other arch (e.g. aarch64 — Microsoft Surface Laptop 7 /
# Snapdragon X, ARM laptops/SBCs) it would bootstrap an x86_64 system, WIPE THE
# DISK, then die on the first chroot exec. Refuse before anything destructive.
# NOT bypassable: an x86_64 install cannot succeed on a non-x86_64 CPU.
is_supported_arch() {
...
checkpoint_validate() {
    ...
    case "${name}" in
        ...
        bootstrap)
            [[ -f "${MOUNTPOINT}/usr/lib/os-release" ]] ;;
        kernel)
...
_scan_partition_for_resume() {
    ...
    if [[ -n "${_RESUME_TEST_DIR:-}" ]]; then
        ...
        if [[ -f "${fake_mp}/tmp/chimera-installer.conf" ]]; then
            _SCAN_HAS_CONFIG=1
        fi
        return 0
    fi
    ...
    if findmnt -rn -S "${part}" &>/dev/null; then
        ...
        if [[ -n "${existing_mp}" ]]; then
            ...
            if [[ -f "${existing_mp}/tmp/chimera-installer.conf" ]]; then
                _SCAN_HAS_CONFIG=1
            fi
            return 0
        fi
    ...
    local mp
    mp=$(mktemp -d "${TMPDIR:-/tmp}/chimera-resume-scan.XXXXXX")
    ...
    if [[ ${mounted} -eq 1 ]]; then
        ...
        if [[ -f "${mp}/tmp/chimera-installer.conf" ]]; then
            _SCAN_HAS_CONFIG=1
        fi
...
_recover_resume_data() {
    ...
    if [[ -n "${_RESUME_TEST_DIR:-}" ]]; then
        ...
        if [[ -f "${fake_mp}/tmp/chimera-installer.conf" ]]; then
            (umask 077; cp "${fake_mp}/tmp/chimera-installer.conf" "${CONFIG_FILE}")
        fi
        return 0
    fi

    local mp
    mp=$(mktemp -d "${TMPDIR:-/tmp}/chimera-resume-recover.XXXXXX")
    local mounted=0
    ...
    if [[ ${mounted} -gt 0 ]]; then
        ...
        if [[ -f "${mp}/tmp/chimera-installer.conf" ]]; then
            (umask 077; cp "${mp}/tmp/chimera-installer.conf" "${CONFIG_FILE}")
...
_infer_from_timezone() {
    ...
    # Chimera: /etc/localtime symlink → zoneinfo
...
_infer_swap_type() {
    ...
    # zram-generator config (dinit service)
    if [[ -f "${mp}/etc/default/zram" ]] || [[ -f "${mp}/etc/dinit.d/zram" ]]; then
        SWAP_TYPE="zram"
        export SWAP_TYPE
        return 0
    fi
...
# _infer_luks — Detect LUKS from crypttab
_infer_luks() {
    local mp="$1"
...
infer_config_from_partition() {
    ...
    if [[ -n "${_RESUME_TEST_DIR:-}" ]]; then
        ...
        if [[ -z "${mp}" ]]; then
            mp=$(mktemp -d "${TMPDIR:-/tmp}/chimera-infer.XXXXXX")
            if mount -o ro "${part}" "${mp}" 2>/dev/null; then
                need_unmount=1
            elif [[ "${fstype}" == "btrfs" ]]; then
                if mount -o ro,subvol=@ "${part}" "${mp}" 2>/dev/null; then
...
# an installed Chimera Linux system. A missing 'disks' checkpoint does NOT mean the
...
_resume_target_has_system() {
    ...
    for opt in "ro,subvol=@" "ro"; do
        if mount -o "${opt}" "${root}" "${probe}" 2>/dev/null; then
            # Chimera markers: an ID=chimera os-release or the apk database
            # (Chimera uses apk, like Alpine, but is otherwise unrelated).
            if grep -q '^ID=chimera' "${probe}/etc/os-release" 2>/dev/null || \
               [[ -d "${probe}/lib/apk/db" ]] || \
               [[ -f "${probe}/etc/chimera-release" ]]; then
                found=0
            fi
