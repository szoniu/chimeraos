...
log_relocate_to_target() {
    ...
    local target_dir="${MOUNTPOINT}/var/log"
    mkdir -p "${target_dir}" 2>/dev/null || return 0

    local target="${target_dir}/chimera-installer.log"
    ...
    LOG_FILE="${target}"
    SKIPPED_LOG="${target_dir}/chimera-installer-skipped.log"
...
skipped_step_record() {
    local desc="$1"
    : "${SKIPPED_LOG:=/tmp/chimera-installer-skipped.log}"
...
skipped_steps_report() {
    local f="${SKIPPED_LOG:-/tmp/chimera-installer-skipped.log}"
    [[ -s "${f}" ]] || return 0
