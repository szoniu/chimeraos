...
config_save() {
    ...
    (
        umask 077
        {
            ...
            echo "# Chimera Linux TUI Installer configuration"
...
config_load() {
    ...
    local safe_file
    safe_file=$(mktemp "${TMPDIR:-/tmp}/chimera-config-safe.XXXXXX")
...
# config_get — Get a config variable value
config_get() {
    local var="$1"
    echo "${!var:-}"
}
...
# config_diff — Compare two config files
config_diff() {
...
validate_config() {
    ...
    local -a required=(
        TARGET_DISK FILESYSTEM HOSTNAME TIMEZONE
    ...
    if [[ -n "${KERNEL_TYPE:-}" ]] && \
       [[ "${KERNEL_TYPE}" != "lts" && "${KERNEL_TYPE}" != "stable" ]]; then
        errors+=("KERNEL_TYPE='${KERNEL_TYPE}' — must be lts or stable")
    fi

    if [[ -n "${BOOTLOADER_TYPE:-}" ]] && \
       [[ "${BOOTLOADER_TYPE}" != "grub" && "${BOOTLOADER_TYPE}" != "systemd-boot" ]]; then
