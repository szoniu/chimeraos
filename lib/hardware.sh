...
# Chimera Linux uses open-source drivers only (no proprietary NVIDIA)
detect_gpu() {
    GPU_VENDOR=""
    GPU_DEVICE_ID=""
    GPU_DEVICE_NAME=""
    ...
    if [[ ${#gpu_lines[@]} -ge 2 ]]; then
        ...
        for (( i=0; i<${#gpu_lines[@]}; i++ )); do
            ...
            # AMD: PCI slot heuristic — bus 00 = iGPU, otherwise dGPU
            if [[ "${slot_bus}" == "00" ]]; then
                igpu_idx=${i}
            else
                dgpu_idx=${i}
...
# correctly (GRUB/systemd-boot menu, console, SDDM/Plasma/GNOME). Also flags
# ALC287 Auto-Mute quirk and GPD fan-daemon need.
#
