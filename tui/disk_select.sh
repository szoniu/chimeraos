...
# _shrink_wizard — Interactive wizard for shrinking a partition to make room for Chimera
...
_shrink_wizard() {
    ...
    dialog_msgbox "No Free Space" \
        "There is not enough free space on ${disk} to create a Chimera partition.\n\n\
...
There is not enough room to free space for Chimera."
        return "${TUI_BACK}"
    fi

    local max_chimera_mib=$(( total_mib - min_part_size ))

    # Default: half available or 20 GiB, whichever is smaller
    local default_chimera=20480
    if [[ $(( max_chimera_mib / 2 )) -lt ${default_chimera} ]]; then
        default_chimera=$(( max_chimera_mib / 2 ))
    fi
    if [[ ${default_chimera} -lt ${CHIMERA_MIN_SIZE_MIB} ]]; then
        default_chimera=${CHIMERA_MIN_SIZE_MIB}
    fi

    # Step 6: Ask for Chimera size
    local chimera_size_mib
    while true; do
        chimera_size_mib=$(dialog_inputbox "Space for Chimera (MiB)" \
            "How much space to free for Chimera?\n\n\
Partition: ${selected_part} (${selected_fstype})\n\
Total:     ${total_mib} MiB\n\
Used:      ${used_mib} MiB\n\
Maximum:   ${max_chimera_mib} MiB\n\
Minimum:   ${CHIMERA_MIN_SIZE_MIB} MiB (8 GiB)\n\n\
Enter size in MiB:" \
            "${default_chimera}") || return "${TUI_BACK}"

        if [[ ! "${chimera_size_mib}" =~ ^[0-9]+$ ]]; then
            dialog_msgbox "Invalid Input" "Please enter a number (MiB)." || true
            continue
        fi

        if [[ ${chimera_size_mib} -lt ${CHIMERA_MIN_SIZE_MIB} ]]; then
            dialog_msgbox "Too Small" \
                "Minimum size for Chimera is ${CHIMERA_MIN_SIZE_MIB} MiB (8 GiB)." || true
            continue
        fi

        if [[ ${chimera_size_mib} -gt ${max_chimera_mib} ]]; then
            dialog_msgbox "Too Large" \
                "Maximum available space is ${max_chimera_mib} MiB.\n\
(Total ${total_mib} - Used ${used_mib} - 1 GiB safety margin)" || true
    ...
    local new_part_size=$(( total_mib - chimera_size_mib ))
  ...
  Freed space:  ${chimera_size_mib} MiB (for Chimera)\n\n\
Proceed with shrink?" \
        || return "${TUI_BACK}"
...
screen_disk_select() {
    ...
    case "${scheme}" in
        dual-boot)
            ...
            # Build rich partition list with OS info
            local -a part_items=()
...
Type ERASE in the next dialog to confirm."
                    ...
                    if [[ ${free_mib} -lt ${CHIMERA_MIN_SIZE_MIB} ]]; then
...
You will then be asked to specify partition paths."
            ...
            PS1="(chimera-partition) \w \$ " bash --norc --noprofile || true
