...
_extra_pkg_state() {
    ...
    case "${pkg}" in
        ...
        gaming)            [[ "${ENABLE_GAMING:-no}" == "yes" ]] && echo "on" && return ;;
...
screen_extra_packages() {
    ...
    # --- Gaming ---
    items+=("gaming" "Gaming (gamescope, Steam udev, gcompat)" "$(_extra_pkg_state gaming off)")
    ...
    export ENABLE_HYPRLAND ENABLE_GAMING ENABLE_FINGERPRINT ENABLE_THUNDERBOLT ENABLE_SENSORS ENABLE_WWAN

    # Build EXTRA_PACKAGES from non-special items
    local -a extra_pkgs=()
    for pkg in ${selected}; do
        case "${pkg}" in
            hyprland-ecosystem|gaming|fprintd|bolt|iio-sensor-proxy|ModemManager) ;;
            *) extra_pkgs+=("${pkg}") ;;
        esac
    done

    # Collect any preset EXTRA_PACKAGES not in the checklist above
    local _known_pkgs="fastfetch btop htop kitty vim git tmux v4l-utils hyprland-ecosystem gaming fprintd bolt iio-sensor-proxy ModemManager"
    local _preset_extra=""
    local _ep
