...
export LOG_FILE="/tmp/chimera-test-shrink.log"
export DRY_RUN=1
...
echo "=== Test: CHIMERA_MIN_SIZE_MIB constant ==="

assert_eq "CHIMERA_MIN_SIZE_MIB is 8192" "8192" "${CHIMERA_MIN_SIZE_MIB}"
...
assert_contains "NTFS plan has partition resize" "Resize partition" "${plan_text}"
assert_contains "NTFS plan has partprobe" "Re-read" "${plan_text}"
...
assert_contains "ext4 plan has partition resize" "Resize partition" "${plan_text}"
...
assert_contains "btrfs plan has partition resize" "Resize partition" "${plan_text}"
...
assert_contains "Dualboot+shrink has partition create" "free space" "${plan_text}"
