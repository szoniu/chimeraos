...
export LOG_FILE="/tmp/chimera-test-multiboot.log"
export DRY_RUN=1
...
TMPFILE="/tmp/chimera-test-multiboot-$$.conf"
config_save "${TMPFILE}"
...
# Should not have parted mkpart (we already have ROOT_PARTITION)
plan_has_create=0
for action in "${DISK_ACTIONS[@]}"; do
    [[ "${action}" == *"free space"* ]] && plan_has_create=1
done
assert_eq "No partition create when ROOT_PARTITION set" "0" "${plan_has_create}"
